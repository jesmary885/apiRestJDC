...
export default {
    ...
    methods: {
        async viewEntity(){
           await this.axios.get(`/api/entities/${this.$route.params.id}`)
            .then(response=>{
                const {name,direction,phone,email,type} = response.data
        ...
        async edit(){
           await this.axios.put(`/api/entities/${this.$route.params.id}`, this.entity)
...
<div class="container">
    <div class="card">
        <h1 class="py-2 text-lg text-gray-600 ml-4 mt-1 font-semibold"> <i class="fas fa-user-tie"></i> Actualizar entidad</h1>
    </div>
